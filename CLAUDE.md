...
- 路由离开根路由、用户亲手点击其他分栏、设置关闭、预算或次数用尽，任一成立即结束会话。
- 离开根路由后重新落回才会再开一次会话，同一次停留不重复开启。

判定依据是当前路由，不是分栏的选中类名。微博只用带构建哈希的类名（如 `_cur_118ye_33`）表示选中态，且类名与路由分别更新，存在类名已指向「最新微博」而内容仍是「全部关注」的中间态；按类名判断会把这种状态读成已经到位并就此收手。
